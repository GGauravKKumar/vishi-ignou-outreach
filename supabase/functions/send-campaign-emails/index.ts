...
/**
 * Sanitizes display names to prevent header injection.
 */
...
async function sendEmailWithRetry(
  ...
  console.log(`[Email] =============================================`);
  console.log(`[Email] Attempting to send email to: ${to.email}`);
  console.log(`[Email]   From: "${from.name}" <${from.email}>`);
  console.log(`[Email]   To: "${to.name}" <${to.email}>`);
  ...
  for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
    try {
      ...
      console.error(`[Retry] Attempt ${attempt + 1}/${MAX_RETRIES} failed for ${to.email}: ${errorMessage}`);
      ...
      if (errorMessage.includes("timed out")) {
        if (attempt === 0) {
          console.log(`[Timeout] Retrying ${to.email} once after timeout`);
          await delay(1000);
          continue;
        }
  ...
  try {
    ...
    for (const recipient of validRecipients) {
      ...
      const result = await sendEmailWithRetry(
        smtpClient,
        { name: sanitizeDisplayName(smtpConfig.from_name), email: smtpConfig.from_email },
        { name: sanitizeDisplayName(recipient.name), email: recipient.email },
